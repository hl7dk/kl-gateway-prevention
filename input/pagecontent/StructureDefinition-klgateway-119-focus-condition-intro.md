### Scope and usage
The  kl-gateway-119-focus-condition holds a diagnosiscode, as it is reported to the municipality in the request for prevention/health promotion from a Hospital or General practitioner using SKS (ICD10) or ICPC2. Nothing else may be reported. This includes diagnoses written in free-text by Hospital, General practitioner or Municipallity staff, as well as codings made by municipality staff.

Be aware, that each 'whole meaning' should have its own instance of kl-gateway-119-focus-condition. A 'whole meaning' may be a single diagnosis code OR a diagnosis code with add-on-codes (tillægskoder) e.g. DK257+MN02BA01 means that a hospital patient has a chronic ulcer caused by the intake of acetylsalicylacid. They are a 'whole meaning' because the codes would loose their meaning if represented in two different focus-condition instanses. The code should be represented in Condition.code.
